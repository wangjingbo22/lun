...
INS_t* INS_Init(void)
{
    ...
    // 初始化速度卡尔曼滤波器 (dt=0.001s, encoder_noise=0.05, imu_noise=2.0)
    extern VelocityKF_t velocityKF;
    VelocityKF_Init(&velocityKF, 0.001f, 0.05f, 2.0f);
...
void INS_Task(void* pvParameters)
{
    ...
    for(;;)
    {
        ...
        if ((count % 1) == 0)
        {
            ...
            // 获取最终数据
            INS.Yaw = QEKF_INS.Yaw / 57.295779513f;
            INS.Pitch = -QEKF_INS.Roll / 57.295779513f;
            INS.Roll = QEKF_INS.Pitch / 57.295779513f;
            INS.YawTotalAngle = QEKF_INS.YawTotalAngle / 57.295779513f;
